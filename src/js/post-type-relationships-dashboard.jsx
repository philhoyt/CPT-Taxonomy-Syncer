...
function PostTypeRelationshipsDashboard( { postType, taxonomy } ) {
	...
	useEffect( () => {
		if ( ! postType || ! taxonomy ) {
			setError(
				__( 'Missing post type or taxonomy.', 'cpt-taxonomy-syncer' )
			);
			setLoading( false );
			return;
		}
	...
	const saveOrder = async ( parentPostId, taxonomySlug, order ) => {
		setSaving( true );
		try {
			await apiFetch( {
	...
	if ( error ) {
		return (
			<div className="cpt-tax-relationships-error notice notice-error">
				<p>
					<strong>{ __( 'Error:', 'cpt-taxonomy-syncer' ) }</strong>{ ' ' }
					{ error }
				</p>
			</div>
		);
	...
	if ( relationships.length === 0 ) {
		return (
			<div className="cpt-tax-relationships-empty">
				<p>
					{ __( 'No relationships found.', 'cpt-taxonomy-syncer' ) }
				</p>
			</div>
		);
	}
					...
					{ relationships.map( ( relationship, relIndex ) => (
						<Fragment key={ relationship.post.id }>
							...
							<tr>
								...
								<td>
									{ relationship.related_posts.length ===
									0 ? (
										<span className="description">
											{ __(
												'No related posts',
												...
												( post, postIndex ) => {
													const isDragged =
														draggedItem?.relationshipIndex ===
															relIndex &&
														draggedItem?.postIndex ===
															postIndex;
													const isDraggedOver =
														draggedOver?.relationshipIndex ===
															relIndex &&
														draggedOver?.postIndex ===
															postIndex;
...
wp.domReady( () => {
	...
	root.render(
		<PostTypeRelationshipsDashboard
			postType={ postType }
			taxonomy={ taxonomy }
		/>
	);
