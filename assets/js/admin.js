...
/* global cptTaxSyncerAdmin, jQuery */
	...
	$( document ).ready( function () {
		...
		$( document ).on( 'click', '.remove-pair', function () {
			...
			if ( $( '#cpt-tax-pairs tbody tr' ).length === 0 ) {
				$( '#cpt-tax-pairs tbody' ).append(
					'<tr class="no-pairs"><td colspan="3">No pairs configured yet.</td></tr>'
				);
			}
		...
		$( '.sync-posts-to-terms' ).on( 'click', function () {
			...
			const progressId =
				'progress-' + cptSlug + '-' + taxonomySlug + '-posts';
			let $progressContainer = $( '#' + progressId );
			if ( $progressContainer.length === 0 ) {
				$progressContainer = $(
					'<div id="' +
						progressId +
						'" class="cpt-tax-sync-progress" style="margin-top: 10px;"></div>'
				);
		...
		$( '.sync-terms-to-posts' ).on( 'click', function () {
			...
			// Show progress container
			const progressId =
				'progress-' + cptSlug + '-' + taxonomySlug + '-terms';
			let $progressContainer = $( '#' + progressId );
			if ( $progressContainer.length === 0 ) {
				$progressContainer = $(
					'<div id="' +
